VoiceControl.configure do |config|
  ...
  config.interpreter = config.api_key.present? ? VoiceControl::Jev.new : DemoInterpreter.new
  ...
  config.group "Go somewhere" do
    %w[overview users orders products reports settings activity].each do |page|
      ...
      config.command "open_#{page}", description: "Open #{page}", aliases: aliases, examples: ["open #{page}"] do
        execute { |_args, _context| VoiceControl::Result.navigate(page == "overview" ? "/" : "/#{page}") }
      end
    end
  end
  ...
  config.group "People & permissions" do
    config.command :give_tokens, description: "Give tokens", aliases: ["give", "add tokens"], examples: ["give user 42 100 tokens"] do
      ...
      execute do |args, _context|
        ...
        raise VoiceControl::InvalidInput, "That demo user does not exist." unless user

        DemoUser.transaction do
          user.with_lock do
            raise VoiceControl::InvalidInput, "That grant exceeds the demo balance limit." if user.token_balance + args[:amount] > 1_000_000_000
        ...
        VoiceControl::Result.event("demo:tokens", args.merge(balance: user.token_balance, total: workspace_settings.tokens_granted))
      end
    end
    config.command :set_plan, description: "Change my account plan",
      ...
      execute do |args, _context|
        ...
        VoiceControl::Result.event("demo:plan", args)
      end
    end
    config.command :announce, description: "Show announcement", aliases: ["announce"], examples: ["show announcement"] do
      ...
      execute { |args, _context| VoiceControl::Result.event("demo:announcement", args) }
    end
  end

  config.group "Workspace tools" do
    config.command :workspace_summary, description: "Show workspace summary", pages: "/settings",
      aliases: ["summarize workspace"], examples: ["summarize workspace"] do
      execute do |_args, _context|
        VoiceControl::Result.message("Workspace: #{workspace_settings.workspace_name}.", notify: "Workspace details ready.")
      end
    end
    config.command :discount, description: "Set discount", aliases: ["set discount"], examples: ["set discount"] do
      ...
      execute do |args, _context|
        ...
        VoiceControl::Result.event("demo:discount", args)
      end
    end
    config.command :notifications, description: "Change notifications", aliases: ["notifications"], examples: ["change notifications"] do
      ...
      execute do |args, _context|
        ...
        VoiceControl::Result.event("demo:notifications", args)
      end
    end
  end
end
